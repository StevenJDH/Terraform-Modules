...
| <a name="input_default_node_pool"></a> [default\_node\_pool](#input\_default\_node\_pool) | The default node pool configuration. Changes to vm\_size forces a rebuild of AKS. | <pre>object({<br>    name                = optional(string)<br>    vm_size             = string<br>    vnet_subnet_id      = optional(string)<br>    node_count          = optional(number)<br>    enable_auto_scaling = optional(bool)<br>    scaling_min_count   = optional(number)<br>    scaling_max_count   = optional(number)<br>  })</pre> | <pre>{<br>  "enable_auto_scaling": false,<br>  "name": "default",<br>  "node_count": 1,<br>  "scaling_max_count": 1,<br>  "scaling_min_count": 1,<br>  "vm_size": "Standard_D2_v2",<br>  "vnet_subnet_id": null<br>}</pre> | no |
